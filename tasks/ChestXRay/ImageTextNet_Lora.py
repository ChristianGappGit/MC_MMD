...
class LLaMAPretrainedModel(Transformer_LLaMA):
    ...
    def init_LLaMA_weights(self, module):
        if isinstance(module, (lora.Linear, ParallelEmbedding)):
            module.weight.data.normal_(mean=0.0, std=self.config.initializer_range)
        elif isinstance(module, torch.nn.LayerNorm):
            module.bias.data.zero_()
            module.weight.data.fill_(1.0)
        if isinstance(module, lora.Linear) and module.bias is not None:
            module.bias.data.zero_()
...
class AttentionLoRA(nn.Module):
    def __init__(self, args: ModelArgs, r: int):
        super().__init__()

        self.n_local_heads = args.n_heads # // fs_init.get_model_parallel_world_size() #TODO: no multiple GPU support yet
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
        ...
        xq, xk, xv = self.wq(x), self.wk(x), self.wv(x) #FIXME: error is in this line
        ...
        if freqs_cis is not None:
            ...
            #print warning
            print("Warning: apply_rotary_emb() was called in AttentionLoRA.forward() method.")
...
class FeedForwardLoRA(nn.Module):
    def __init__(
        ...
        self.w2 = lora.Linear(
            hidden_dim, dim, r=r        #TODO: check if this makes sense
        )
        self.w3 = lora.Linear(
            dim, hidden_dim, r=r
        )
...
class TransformerBlock_LLaMA_LoRA(nn.Module):           #adapted from llama/model.py
    def __init__(self, layer_id: int, args: ModelArgs, r: int):
        super().__init__()
        self.n_heads = args.n_heads
        self.dim = args.dim
        self.head_dim = args.dim // args.n_heads
        self.attention = AttentionLoRA(args, r)
        self.feed_forward = FeedForwardLoRA(
            dim=args.dim, hidden_dim= 4 * args.dim, multiple_of=args.multiple_of, r=r
        )
...
class LLaMAOutput(FeedForwardLLaMA): #TODO: could also use FeedForwardLoRA here.
    """LLaMA output layer.
    Based on: LLaMA
    """
...
class CrossAttentionBlockLLaMA(nn.Module):
    ...
    Based on: LLaMA
    """
    def __init__(self, args: ModelArgs, r: int) -> None:
        super().__init__()
        self.attention_x = CrossAttentionLLaMA(args,r)
        self.attention_y = CrossAttentionLLaMA(args,r)
        self.attention_norm = RMSNormLLaMA(args.dim, eps=args.norm_eps)
...
class MultiModalTransformerLLaMA(LLaMAPretrainedModel):
    ...
    def __init__(
        ...
        #self.embeddings = lora.Embedding(num_embeddings=self.config.num_embeddings,     #TODO: should be ok
        #                                    embedding_dim=self.config.embedding_dim, r=llama_config["r"]) #FIXME: doesnt work properly yet
        params = {  "dim": llama_config["embedding_dim"],
                    "multiple_of": llama_config["multiple_of"],
                    "n_heads": llama_config["num_attention_heads"],
                    "n_layers": llama_config["n_layers"],
        ...
        #)
        self.output = lora.Linear(model_args.dim, model_args.vocab_size, r=llama_config["r"])
        self.freqs_cis = precompute_freqs_cis(
            self.params.dim // self.params.n_heads, self.params.max_seq_len * 2
        )
        self.text_encoder = nn.ModuleList([TransformerBlock_LLaMA_LoRA(layer, model_args, llama_config["r"]) for layer in range(num_text_layers)])
        self.vision_encoder = nn.ModuleList([TransformerBlock_LLaMA_LoRA(layer, model_args, llama_config["r"]) for layer in range(num_vision_layers)])
        self.cross_attention_encoder = nn.ModuleList([CrossAttentionBlockLLaMA(model_args, llama_config["r"]) for _ in range(num_cross_attention_layers)]) #FIXME: check if parallel even works or other functions must be implemented for llama
...
class ImageTextNet_Lora(torch.nn.Module):
    """
    ...
    def  __init__(
        ...
        r: int = 16, #added, used for r in lora.Linear class
    ) -> None:
        """
        Args:
        ...
        if model_type == "bert":
            ...
            self.cls_head = lora.Linear(hidden_size, num_classes, r=r) #FIXME ColumnParallelLinear(hidden_size, num_classes) #
