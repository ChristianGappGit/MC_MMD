...
__all__ = ("Transformer_LLaMA_LoRA", "LLaMAPretrainedModel", "BertPreTrainedModel", "AttentionLoRA", "FeedForwardLoRA",
            "TransformerBlock_LLaMA_LoRA", "CrossAttentionBert", "BertOutput", "CrossAttentionLLaMA",
            "LLaMAOutput", "CrossAttentionBlockLLaMA", "CrossAttentionLayerBert",
            "PoolerLLaMA", "PoolerBert", "MultiModalTransformerLLaMA", "MultiModalTransformerBert" ,"ImageTextNet_Peft")


class Transformer_LLaMA_LoRA(nn.Module):           #adapted from llama/model.py
    def __init__(self, params: ModelArgs):
        ...
        for layer_id in range(params.n_layers):
            self.layers.append(TransformerBlock_LLaMA_LoRA(layer_id, params))

        self.norm = RMSNormLLaMA(params.dim, eps=params.norm_eps)
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int, vision: bool = False): #TODO: done: added bool vision
        #print("tokens.shape", tokens.shape) #TODO: delete
        if not vision: #text
            bsz, seqlen = tokens.shape
            h = self.tok_embeddings(tokens)
        else: #vision
            bsz,seqlen, dim = tokens.shape
            h = tokens #vision_features
        ...
        #print("h_shape from Transformer",h.shape)    #TODO: delete
        output = self.output(h[:, -1, :])  # only compute last logits
        #print("output_shape from Transformer",output.shape)    #TODO: delete
        #print("output from Transformer",output)    #TODO: delete
        return output.float(), h #changed from output.float() to output.float(), h


class LLaMAPretrainedModel(Transformer_LLaMA_LoRA):    #TODO: must be changed to Transformer_LLaMA_LoRA
    """
    ...
    def from_pretrained(
        ...
        num_pre_activation_layers,
        num_pre_activation_layers_cross,
        llama_config,
        ...
        **kwargs,
        ...
        def load(
            ...
            model = cls(num_vision_layers, num_text_layers, num_cross_attention_layers, num_pre_activation_layers, num_pre_activation_layers_cross,
                         llama_config, model_args, *inputs, **kwargs)
...
class AttentionLoRAVision(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.wq_vision = nn.Linear(
            args.dim,
            args.n_heads * self.head_dim,
        )
        self.wk_vision = nn.Linear(
            args.dim,
            args.n_heads * self.head_dim,
        )
        self.wv_vision = nn.Linear(
            args.dim,
            args.n_heads * self.head_dim,
        )
        self.wo_vision = nn.Linear(
            args.n_heads * self.head_dim,
            args.dim,
        )
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
        ...
        xq, xk, xv = self.wq_vision(x), self.wk_vision(x), self.wv_vision(x)
...
class AttentionLoRA(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
        #print("Attention(): x_shape",x.shape)    #TODO: delete this line
        #print("x", x) #TODO: delete this line
        bsz, seqlen, _ = x.shape
        #print("wq",self.wq.weight) #TODO: delete this line
        #print("wk",self.wk.weight) #TODO: delete this line
        #print("wv",self.wv.weight) #TODO: delete this line
        ...
        #print("Attention(): key: ", xk)    #TODO: delete this line
        #print("Attention(): value: ", xv)    #TODO: delete this line
...
class FeedForwardLoRAVision(nn.Module):
    def __init__(
        self,
        dim: int,
        hidden_dim: int,
        ...
        self.w1_vision = nn.Linear(
            dim, hidden_dim,
        )
        self.w2_vision = nn.Linear(
            hidden_dim, dim,       #TODO: check if this makes sense
        )
        self.w3_vision = nn.Linear(
            dim, hidden_dim, 
        )

    def forward(self, x):
        return self.w2_vision(F.silu(self.w1_vision(x)) * self.w3_vision(x))
...
class TransformerBlock_LLaMA_LoRA(nn.Module):           #adapted from llama/model.py
    def __init__(self, layer_id: int, args: ModelArgs, modality: str=None): #modality defines layer names in AttentionLoRA
        ...
        self.head_dim = args.dim // args.n_heads
        if modality == "vision":
            self.attention = AttentionLoRAVision(args)
            self.feed_forward = FeedForwardLoRAVision(
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
        ...
        #print("h_shape from TransformerBlock",h.shape)    #TODO: delete
        #print("h in Attention()", h)
        out = h + self.feed_forward.forward(self.ffn_norm(h))
        #print("h_shape from TransformerBlock",h.shape)    #TODO: delete
        #print("out_shape from TransformerBlock",out.shape)    #TODO: delete
        return out
...
class CrossAttentionBert(nn.Module):
    ...
    def __init__(self, config) -> None:
        ...
        self.key = nn.Linear(config.hidden_size, self.all_head_siz)
...
class CrossAttentionLLaMA(nn.Module):
    """LLaMA Attention layer.
    Based on: LLaMA
    """
    def __init__(self, args: ModelArgs,):
        super().__init__()
        self.n_local_heads = args.n_heads #// fs_init.get_model_parallel_world_size() #FIXME: does not work for multiple GPU yet
        self.head_dim = args.dim // args.n_heads
    ...
    def forward(self, hidden_states, context, start_pos: int, freqs_cis=None, mask=None): #TODO: check and modify if necessary
        ...
        #print("Attention(): output_shape", output.shape)
...
class PoolerLLaMA(nn.Module):
    ...
    def __init__(self, in_features, out_features) -> None:
        super().__init__()
        self.densePooler = nn.Linear(in_features, out_features)
        #or self.densePooler = nn.Linear(nn.Linear(in_features, in_features), out_features)
        self.activation = nn.Tanh()

    def forward(self, hidden_states):
        first_token_tensor = hidden_states
        #print("first_token_tensor.shape",first_token_tensor.shape)    #TODO: delete
        #print("first_token_tensor.type()",first_token_tensor.type())    #TODO: delete
        pooler_output = self.densePooler(first_token_tensor)
        #print("pooler_output",pooler_output)    #TODO: delete
        pooler_output = self.activation(pooler_output)
        #print("pooler_output",pooler_output)    #TODO: delete
        return pooler_output
    
class ClsLLaMA(nn.Module):
    """LLaMA pooler layer.
    Based on: LLaMA
    """
    ...
    def forward(self, hidden_states):
        ...
        #print("cls_output",cls_output)    #TODO: delete
...
class MultiModalTransformerLLaMA(LLaMAPretrainedModel):
    ...
    def __init__(
        self, num_vision_layers: int, num_text_layers: int, num_cross_attention_layers: int, num_pre_activation_layers: int, 
        num_pre_activation_layers_cross: int, llama_config: dict, model_args: ModelArgs
    ) -> None:
        print("------------------------------------------------------")
        ...
        self.embeddings = torch.nn.Embedding(num_embeddings=self.config.num_embeddings, embedding_dim=self.config.embedding_dim) #TODO: check if works properly
        ...
        self.serial_pipeline = llama_config["serial_pipeline"]
        self.use_llama_pre_pipeline = llama_config["n_layers"] > 0
        self.text_only = llama_config["text_only"]
        self.vision_only = llama_config["vision_only"]
        self.text_encoder = nn.ModuleList([TransformerBlock_LLaMA_LoRA(layer, model_args,"text") for layer in range(num_text_layers)])
        self.vision_encoder = nn.ModuleList([TransformerBlock_LLaMA_LoRA(layer, model_args,"vision") for layer in range(num_vision_layers)])
        self.cross_attention_encoder = nn.ModuleList([CrossAttentionBlockLLaMA(model_args,) for _ in range(num_cross_attention_layers)])
        self.pre_activation_encoder = nn.ModuleList([TransformerBlock_LLaMA_LoRA(layer, model_args, "") for layer in range(num_pre_activation_layers)])
        self.pre_activation_cross_encoder = nn.ModuleList([CrossAttentionBlockLLaMA(model_args,) for _ in range(num_pre_activation_layers_cross)])
    ...
    def forward(self, input_ids, start_pos: int, token_type_ids=None, vision_features=None, attention_mask=None,
        ...
        #freqs_cis_vision = self.freqs_cis[start_pos : start_pos + vision_features.shape[1]]   #FIXME: check if vision_features.shape[1] is correct
        ...
        #llama pre pipeline model: #TODO: check if this is the proper "model()"
        if self.use_llama_pre_pipeline:
            if text_available:
                outputs_llama_text, text_features = super().forward(tokens=input_ids, start_pos=start_pos, vision=False) #__call__() llama model
            if vision_available:
                outputs_llama_vision, vision_features = super().forward(tokens=vision_features, start_pos=start_pos, vision=True) #__call__() llama model
        
        if self.text_only or (text_available and not vision_available):
            if not len(self.text_encoder) > 0: #text modality
                raise ValueError("Text_encoder must be non-empty.")
            for layer in self.text_encoder:
                text_features = layer(text_features, start_pos, freqs_cis=freqs_cis_text, mask=attention_mask)
        elif self.vision_only or (vision_available and not text_available):
            if not len(self.vision_encoder) > 0: #vision modality
                raise ValueError("Vision_encoder must be non-empty.")
            ...
            #init pre_features as a copy of features
            text_features_pre = text_features.clone()
            vision_features_pre = vision_features.clone()

            #print("text_features_pre.shape", text_features_pre.shape)
            ...
            for layer in self.text_encoder:
                ...
                #print("text_features_shape", text_features.shape)
                #print("text_features", text_features)
                text_features = layer(text_features, start_pos, freqs_cis=freqs_cis_text, mask=attention_mask) #[0] ... was wrong #FIXME: check if mask is correct
            
            for layer in self.vision_encoder:
                #print("-----------")
                #print("vision_features_shape", vision_features.shape)
                #print("vision_features", vision_features)
                vision_features = layer(vision_features, start_pos, freqs_cis=freqs_cis_vision, mask=None) #[0] mask must be None
            ...
            if len(self.cross_attention_encoder) > 0:
                if self.serial_pipeline:
                    ...
                    #init cross_features
                    vision_features_cross = vision_features_pre #beware that *pre_features will be changed herein afterwards
                    text_features_cross = text_features_pre
                    for layer in self.cross_attention_encoder:
                        vision_features_cross, text_features_cross = layer(vision_features_cross, text_features_cross, start_pos, freqs_cis=None, mask=None) #FIXME: check freqs_cis
                    #summation
            ...
            for layer in self.pre_activation_cross_encoder:
                vision_features, text_features = layer(vision_features, text_features, start_pos, freqs_cis=None, mask=None)

        else: #no modality available
            raise ValueError("No modality available.")

        #for both single- and multimodality:
        if vision_available:
        ...
        if text_available:
            ...
            output_text = self.output(text_features[:, -1, :])
        else: 
            output_text = None
        ...
        #print("vision_features[:, -1, :]", vision_features[:, -1, :])
        #print("text_features[:, -1, :]",text_features[:, -1, :])
        #print("output_text", output_text)
        #print("output_vision.type()", output_text.type())
        #print("output_vision_shape", output_vision.shape)
        #print("output_text_shape", output_text.shape)
        #print("output_text", output_text)

        return output_vision, output_text #add output_llama ???
...
class ImageTextNet_Peft(torch.nn.Module):
    """
    ...
    def  __init__(
        ...
        num_pre_activation_layers: int, #only for llama relevant
        ...
        model_type: str = "LLaMA7B", #possible options: LLaMA7B, LLaMA13B, LLaMA30B, bert
        ...
        llama_config = {
            ...
            "norm_eps": 1e-6,
            "max_seq_len": 512, #TODO: potential error source
            "max_batch_size": 32,
        ...
        if model_type == "bert":
            ...
            #torch.set_default_tensor_type(torch.cuda.HalfTensor)
            self.pooler = PoolerLLaMA(llama_config["num_embeddings"], dim)
            self.cls_head = ClsLLaMA(dim, num_classes)
    ...
    def forward(self, input_ids, vision_features=None, token_type_ids=None):   ##__call__() #token_type_ids not used for llama
        ...
        #print("vision_features_shape",vision_features.shape)
        use_vision = (vision_features is not None)
        if use_vision:
            vision_features = self.vision_proj(vision_features)
            #print("vision_features_shape",vision_features.shape)
            vision_features = vision_features.flatten(2).transpose(1,2) #FIXME: flatten: reshaping it into 1-dimensional tensor
            #print("vision_features_shape",vision_features.shape)
        ...
        #print("Pooling...")
        #print(f"hidden_state_text.shape = {hidden_state_text.shape}")
        #print(f"hidden_state_vision.shape = {hidden_state_vision.shape}")
            #add usage of hidden_state vision to pooler what also affects logits
        pooled_text_features = self.pooler(hidden_state_text) if hidden_state_text is not None else None #dense + activation (Tanh)
        pooled_vision_features = self.pooler(hidden_state_vision) if hidden_state_vision is not None else None
        #print(f"pooled_text_features.shape = {pooled_text_features.shape}")
        #print(f"pooled_vision_features.shape = {pooled_vision_features.shape}")

        if use_text and not use_vision:
            logits = self.drop(pooled_text_features)
        elif use_vision and not use_text:
        ...
        logits = self.cls_head(logits) #dense + activation (Sigmoid)
        #print("logits.shape after cls", logits.shape)
        #print("logits_type", logits.type())
        #print(logits)
